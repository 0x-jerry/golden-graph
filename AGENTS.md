...
- **Theme** → `packages/renderer/src/theme/`. `GraphTheme` is the single source of truth: `default.ts` holds the shipped values, views read `theme.*`, and `applyThemeToElement` mirrors them as `--gr-*` CSS vars for the Vue chrome. `setTheme()` hot-swaps live. New tokens need a default in `default.ts` and a mirror in partial patches (e.g. playground `DARK_THEME`). Geometry and a few paint values (stroke widths, tag radius) stay compile-time in `renderer/constants.ts`.
